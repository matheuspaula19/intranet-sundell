<?php
	...
	//monta array de palavras a serem pesquisadas
	$exp = explode(" ", addslashes($q)); 
	$busca = implode("|", $exp); 
	
	//faz busca das palavras pesquisadas
	$busca = "SELECT * FROM news_tab WHERE `titulo` REGEXP '(".$busca.")'  OR  `snippet` REGEXP '(".$busca.")' OR  `noticia` REGEXP '(".$busca.")' order by -data";
	$total_reg = $config_paginacao; // numero de registros por pagina
	if ($pagina=="") {
		$pagina = "1";
		$pc = "1";
	} else {
	...
	if($inicio == 0){$mostrando = 10;}else{$mostrando =  $tr - $inicio;}
...
<html>
	<head>
		<title><?=$q?> - Intranet Sundell - Pesquisa</title>
	...
	<body>
		<div align="center">
			...
			<table class="coltables">
				<tr>
					<td valign="top">
					<span class="col-title" style="font-size: 17px;">Resultados da Pesquisa: <?php echo resume($q,50);?></span>
					<div class="widget-title">
						...
						<li><table><tr><td><input type="text" id="search_box" class="campo_padrao" value="<?=$q?>" name="q" size="25" onkeydown="pesquisa_enter()" style="width: 550px;"></td><td><div id="input_btn" onclick="search()" title="Pesquisar"></div></td></tr></table></li>
						<?php
							if($tr == 1){
								echo "<li><span style=\"font-size: 12px;margin-left: 10px;\">Sua pesquisa retornou $tr resultado.</span></li>";
							}
							else{
								if($tr != 0){
									echo "<li><span style=\"font-size: 12px;margin-left: 10px;\">Mostrando $mostrando de $tr resultados.</span></li>";
								}	
							}
						?>
						</ul>
						<ul id="sundell_news" style="padding: 0;min-height: 490px;">
						<?php
						...
						<table width="100%" id="post_footer">
							...
							<?php
								...
								$proximo = $pc +1;
								if ($pagina>1) {
									echo "<td align=\"left\"><a href='search.php?q=".$q."&p=$anterior'  style=\"color:#ca2900;font-size:12px;text-decoration:none;\">P·gina Anterior</a></td>";
								}
								else{
								...
								if ($pagina<$tp) {
									echo "<td align=\"right\"><a href='search.php?q=".$q."&p=$proximo' style=\"color:#ca2900;font-size:12px;text-decoration:none;\">PrÛxima p·gina</a></td>";
								}
								else
								{
								...
								<?php
									...
									echo "<li><div style=\"padding: 5px;margin: 5px;border: 1px solid rgb(255,204,51);background-color: rgb(255,244,194);\">Sem resultados</li></div>";
								}
							?>
				...
				<td style="width: 302px;" valign="top">
					...
					<div id="latest-news" class="widget-title" style="width: 290px;min-height: 390px;">
						<?php
